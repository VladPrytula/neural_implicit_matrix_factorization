...
class BaseDeepRecommender():
    def __init__(self, config: DefaultDict):
        ...
        self._writer = SummaryWriter(log_dir='runs/{}'.format(
            config['alias']+str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))))
    ...
    def train_epoch(self, data_loader, epoch_num):
        ...
        for batch_idx, batch_data in enumerate(data_loader):
            ...
            if batch_idx % 500 == 0:
                logger.info("Epoch {} has batch {} loss {}".format(
    ...
    def evaluate_epoch(self, eval_data, epoch_num):
        ...
        with torch.no_grad():
            test_users, test_items, negative_interactions_users, negative_items = eval_data[
                0], eval_data[1], eval_data[2], eval_data[3]
            if self._config['use_cuda']:
                test_users = test_users.cuda(non_blocking=True)
                test_items = test_items.cuda(non_blocking=True)
                negative_interactions_users = negative_interactions_users.cuda(non_blocking=True)
            ...
            self._evaluator.set_interactions(test_users.view(-1).tolist(),
                                             test_items.view(-1).tolist(),
                                             test_interactions_scores.view(
                                                 -1).tolist(),
                                             negative_interactions_users.view(
                                                 -1).tolist(),
